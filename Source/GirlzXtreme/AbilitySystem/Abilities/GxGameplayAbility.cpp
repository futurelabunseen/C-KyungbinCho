	...
	: Super(ObjectInitializer)
{
	...
	ActivationPolicy = EGxAbilityActivationPolicy::OnInputTriggered;
}
...
AController* UGxGameplayAbility::GetControllerFromActorInfo() const
{
	if (CurrentActorInfo)
	{
...
void UGxGameplayAbility::OnGiveAbility(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec)
{
...
void UGxGameplayAbility::OnRemoveAbility(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec)
{
	K2_OnAbilityRemoved();
...
bool UGxGameplayAbility::CheckCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, OUT FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	for (TObjectPtr<UGxAbilityCost> AdditionalCost : AdditionalCosts)
	{
...
void UGxGameplayAbility::ApplyCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo) const
{
	...
	auto DetermineIfAbilityHitTarget = [&]()
	{
		...
		{
			if (UGxAbilitySystemComponent* ASC = Cast<UGxAbilitySystemComponent>(ActorInfo->AbilitySystemComponent.Get()))
			{
...
bool UGxGameplayAbility::DoesAbilitySatisfyTagRequirements(const UAbilitySystemComponent& AbilitySystemComponent, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, OUT FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	const UGxAbilitySystemComponent* GxASC = Cast<UGxAbilitySystemComponent>(&AbilitySystemComponent);
	...
	if (GxASC)
	{
		GxASC->GetAdditionalActivationTagRequirements(AbilityTags, AllRequiredTags, AllBlockedTags);
	}
	...
	if (AllBlockedTags.Num() || AllRequiredTags.Num())
	{
		...
		{
			if (OptionalRelevantTags && AbilitySystemComponentTags.HasTag(GxGameplayTags::Status_Death))
			{
				// If player is dead and was rejected due to blocking tags, give that feedback
				OptionalRelevantTags->AddTag(GxGameplayTags::Ability_ActivateFail_IsDead);
			}
...
void UGxGameplayAbility::OnPawnAvatarSet()
{
	K2_OnPawnAvatarSet();
}

void UGxGameplayAbility::GetAbilitySource(FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, float& OutSourceLevel, const IGxAbilitySourceInterface*& OutAbilitySource, AActor*& OutEffectCauser) const
{
	OutSourceLevel = 0.0f;
	...
	OutAbilitySource = Cast<IGxAbilitySourceInterface>(SourceObject);
}

void UGxGameplayAbility::TryActivateAbilityOnSpawn(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec) const
{
	const bool bIsPredicting = (Spec.ActivationInfo.ActivationMode == EGameplayAbilityActivationMode::Predicting);

	// Try to activate if activation policy is on spawn.
	if (ActorInfo && !Spec.IsActive() && !bIsPredicting && (ActivationPolicy == EGxAbilityActivationPolicy::OnSpawn))
	{
